...
- **정사각**(1:1)으로 만든다. 카드에서 184px로 렌더링되므로 최소 368px, 권장 800px.
- 목록 카드에서는 **우측**에 붙는다. 이미지가 있는 글과 없는 글이 섞여도 제목 시작 위치가 안 흔들리게 하기 위해서다. **좌측으로 옮기지 말 것.**
- 포스트 상세에서는 헤더 아래 전체 폭으로 나온다.
- 없으면 카드는 텍스트만. 목록이 깨지지 않는다.
